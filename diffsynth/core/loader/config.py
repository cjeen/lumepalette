...
class ModelConfig:
    ...
    def parse_download_source(self):
        """
        Decide which backend to use for downloading.

        Priority:
        1) Explicit `download_source` argument on the ModelConfig
        2) Environment variable `DIFFSYNTH_DOWNLOAD_SOURCE`
        3) If any HuggingFace endpoint hint is set (`HF_ENDPOINT` or `DIFFSYNTH_HF_ENDPOINT`),
           prefer HuggingFace so users can switch to mirrors without touching code.
        4) Fallback to ModelScope to keep backward compatibility.
        """
        if self.download_source is not None:
            return self.download_source
        env_source = os.environ.get("DIFFSYNTH_DOWNLOAD_SOURCE")
        if env_source is not None:
            return env_source
        if os.environ.get("HF_ENDPOINT") is not None or os.environ.get("DIFFSYNTH_HF_ENDPOINT") is not None:
            return "huggingface"
        return "modelscope"
    ...
    def download(self):
        ...
        if download_source.lower() == "modelscope":
            if snapshot_download is None:
                raise ImportError("`modelscope` is required for download_source='modelscope', please install it or set download_source='huggingface'.")
            snapshot_download(
                self.model_id,
            ...
            if hf_snapshot_download is None:
                raise ImportError("`huggingface_hub` is required for download_source='huggingface'. Please install it or set download_source='modelscope'.")
            # Allow user to switch to a mirror via DIFFSYNTH_HF_ENDPOINT while
            # keeping official Hugging Face as the default endpoint.
            if os.environ.get("HF_ENDPOINT") is None and os.environ.get("DIFFSYNTH_HF_ENDPOINT") is not None:
                os.environ["HF_ENDPOINT"] = os.environ["DIFFSYNTH_HF_ENDPOINT"]
